...
func init() {
	yai.OnAppStart(func() {
		...
		if expireStr, found := yai.Config.String("cache.expires"); found {
			var err error
		...
		if yai.Config.BoolDefault("cache.memcached", false) && yai.Config.BoolDefault("cache.redis", false) {
			panic("You've configured both memcached and redis, please only include configuration for one cache!")
		}

		// Use memcached?
		if yai.Config.BoolDefault("cache.memcached", false) {
			hosts := strings.Split(yai.Config.StringDefault("cache.hosts", ""), ",")
			if len(hosts) == 0 {
		...
		if yai.Config.BoolDefault("cache.redis", false) {
			hosts := strings.Split(yai.Config.StringDefault("cache.hosts", ""), ",")
			if len(hosts) == 0 {
			...
			password := yai.Config.StringDefault("cache.redis.password", "")
