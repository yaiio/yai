...
const (
	YAI_IMPORT_PATH = "bitbucket.org/appsynth/go-platform/yai"
)
...
func (r *yaiLogs) Write(p []byte) (n int, err error) {
...
var (
	...
	ApiPath    string // e.g. "/Users/robfig/gocode/src/corp/sample/api"
	ViewsPath  string // e.g. "/Users/robfig/gocode/src/corp/sample/api/views"
	...
	// Yai installation details
	YaiPath string // e.g. "/Users/robfig/gocode/src/yai"
	...
	error_log = yaiLogs{c: colors["error"], w: os.Stderr}
...
// Init initializes Yai -- it provides paths for getting around the app.
//
// Params:
//   mode - the run mode, which determines which app.conf settings are used.
//   importPath - the Go import path of the application.
//   srcPath - the path to the source directory, containing Yai and the app.
...
func Init(mode, importPath, srcPath string) {
	...
	var yaiSourcePath string // may be different from the app source path
	if SourcePath == "" {
		yaiSourcePath, SourcePath = findSrcPaths(importPath)
	} else {
		// If the SourcePath was specified, assume both Yai and the app are within it.
		SourcePath = path.Clean(SourcePath)
		yaiSourcePath = SourcePath
		packaged = true
	}

	YaiPath = path.Join(yaiSourcePath, filepath.FromSlash(YAI_IMPORT_PATH))
	BasePath = path.Join(SourcePath, filepath.FromSlash(importPath))
	ApiPath = path.Join(BasePath, "api")
	ViewsPath = path.Join(ApiPath, "views")

	CodePaths = []string{ApiPath}

	ConfPaths = []string{
		path.Join(BasePath, "conf"),
		path.Join(YaiPath, "conf"),
	}

	TemplatePaths = []string{
		ViewsPath,
		path.Join(YaiPath, "templates"),
	}
	...
	CookiePrefix = Config.StringDefault("cookie.prefix", "YAI")
	...
	Initialized = true
	INFO.Printf("Initialized Yai v%s (%s) for %s", VERSION, BUILD_DATE, MINIMUM_GO)
}
...
func getLogger(name string) *log.Logger {
	...
	switch output {
	case "stdout":
		newlog = yaiLogs{c: colors[name], w: os.Stdout}
		logger = newLogger(&newlog)
	case "stderr":
		newlog = yaiLogs{c: colors[name], w: os.Stderr}
		logger = newLogger(&newlog)
	default:
		if output == "off" {
			output = os.DevNull
...
// findSrcPaths uses the "go/build" package to find the source root for Yai
// and the app.
func findSrcPaths(importPath string) (yaiSourcePath, appSourcePath string) {
	var (
	...
	yaiPkg, err := build.Import(YAI_IMPORT_PATH, "", build.FindOnly)
	if err != nil {
		ERROR.Fatalln("Failed to find Yai with error:", err)
	}

	return yaiPkg.SrcRoot, appPkg.SrcRoot
}
...
func addModule(name, importPath, modulePath string) {
	...
	if importPath == Config.StringDefault("module.testrunner", "github.com/yai/modules/testrunner") {
...
func CheckInit() {
	if !Initialized {
		panic("Yai has not been initialized!")
	}
}
