...
func NewRedisCache(host string, password string, defaultExpiration time.Duration) RedisCache {
	var pool = &redis.Pool{
		MaxIdle:     yai.Config.IntDefault("cache.redis.maxidle", 5),
		MaxActive:   yai.Config.IntDefault("cache.redis.maxactive", 0),
		IdleTimeout: time.Duration(yai.Config.IntDefault("cache.redis.idletimeout", 240)) * time.Second,
		Dial: func() (redis.Conn, error) {
			protocol := yai.Config.StringDefault("cache.redis.protocol", "tcp")
			toc := time.Millisecond * time.Duration(yai.Config.IntDefault("cache.redis.timeout.connect", 10000))
			tor := time.Millisecond * time.Duration(yai.Config.IntDefault("cache.redis.timeout.read", 5000))
			tow := time.Millisecond * time.Duration(yai.Config.IntDefault("cache.redis.timeout.write", 5000))
